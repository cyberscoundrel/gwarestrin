// neo4j MCP end-to-end probe: drive a running agent through the pi-mcp-adapter
// to fetch the graph schema and run a cypher read against the homelab graph.
// usage: node scripts/neo4j-probe.mjs <agentId> [baseUrl]
...
console.log(
  ...
  (
    await turn(
      'You have an `mcp` proxy tool connected to MCP servers, one named "neo4j". Use it: call it with {"search": "schema"} to find the neo4j schema tool (likely neo4j_get_neo4j_schema), then CALL that tool. Reply with the node labels it reports.',
    )
  ).slice(0, 500),
);
...
console.log(
  ...
  (
    await turn(
      'Now use the neo4j read tool (likely neo4j_read_neo4j_cypher) via the mcp proxy to run this exact query: MATCH (s:Service)-[:RUNS_ON]->(m:Machine) RETURN s.name, m.name — and reply with the pairs it returns.',
    )
  ).slice(0, 500),
);
