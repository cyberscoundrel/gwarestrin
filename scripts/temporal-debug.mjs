...
async function q(command, language = "sql") {
  ...
  if (j.result?.isError) return { TOOL_ERROR: j.result.content[0].text.slice(0, 200) };
  const parsed = JSON.parse(j.result?.content?.[0]?.text ?? "{}");
  return parsed.rows ?? parsed;
}

// ensure wrench exists first
await fetch(MCP, {
  method: "POST",
  headers: { "content-type": "application/json", accept: "application/json, text/event-stream" },
  body: JSON.stringify({ jsonrpc: "2.0", id: 0, method: "tools/call", params: { name: "upsert_entities", arguments: { entities: [{ name: "__probe__wrench", labels: ["Tool"], properties: { arrived_at: "2026-08-20" }, facets: { temporal: "arrived 2026-08-20" } }] } } }),
});
